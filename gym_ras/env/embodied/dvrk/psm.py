...
class SinglePSM():
    ...
    def get_obs(self): 
        tip_pos, tip_quat = T2Quat(Frame2T(self._psm.measured_cp()))
    ...
    def _set_action(self, action: np.ndarray):
        ...
        pose_world[:3, 3] = pose_world[:3, 3] + action[:3]
        ...
        if self._action_mode == 'yaw':
            ...
            rot[2] = wrapAngle(rot[2] + action[3])  # only change yaw
        ...
        pos = np.clip(np.array(pos), self.workspace_limit[:,0], self.workspace_limit[:,1])
        pos = pos.tolist()
